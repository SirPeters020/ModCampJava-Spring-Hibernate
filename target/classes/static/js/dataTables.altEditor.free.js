    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _openEditModal: function () {
                ...
                var data = this.createDialog(columnDefs, 'Editar Campo', 'Salvar', 'Fechar', 'editRowBtn');
            ...
            _openDeleteModal: function () {
                ...
                $(selector).on('show.bs.modal', function () {
                    var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Fechar</button>' +
                        '<button type="button"  data-content="remove" class="btn btn-danger" id="deleteRowBtn">Deletar</button>';
                    $(selector).find('.modal-title').html('Deletar Campo');
            ...
            _openAddModal: function () {
                var dt = this.s.dt;
                var columnDefs = this.completeColumnDefs();
                var data = this.createDialog(columnDefs, 'Adicionar Campo', 'Adicionar', 'Fechar', 'addRowBtn');
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass) {
                ...
                var selector = this.modal_selector;
                $(selector).on('show.bs.modal', function () {
                    var btns = '<button type="submit" data-content="remove" class="btn btn-default" data-dismiss="modal">'+closeCaption+'</button>' +
                        '<button type="submit"  data-content="remove" class="btn btn-primary" id="editRowBtn">'+buttonCaption+'</button>';
...
//console.log(rowDataArray); //DEBUG
            ...
            onEditRow: function(dt, rowdata, success, error) {
                $.ajax({
                    type: 'POST',
                    url: 'http://localhost:8080/controlecampanha',
                    success: function (e){
                        console.log("Sucesso!!")
                    },
                });
